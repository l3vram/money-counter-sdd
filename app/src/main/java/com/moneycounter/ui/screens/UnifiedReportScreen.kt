...
import com.moneycounter.domain.uniteMovements
...
/** Reporte seleccionable: consolida los movimientos de caja (VENTA/COBRO) que el
 *  usuario marcó en el Historial — total, productos y denominaciones agrupados. */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun UnifiedReportScreen(
    viewModel: MoneyCounterViewModel,
    selectedMovementIds: List<String>,
    onNavigateBack: () -> Unit
) {
    val uiState by viewModel.uiState.collectAsState()
    val context = LocalContext.current
    val selected = uiState.movements.filter { it.id in selectedMovementIds.toSet() }
    val currency = uiState.currencies.firstOrNull { it.id == selected.firstOrNull()?.currencyId }
    val currencyCode = currency?.code.orEmpty()
    val symbol = currency?.symbol ?: "$"
    val united = remember(selected, uiState.currencies) {
        runCatching { uniteMovements(selected, currencyCode) }
    }
        ...
        when {
            ...
            united.isFailure -> {
                ...
                LazyColumn(
                    ...
                    item {
                        LuisoStatCard(
                            label = "$symbol (${u.currencyCode})",
                            value = formatMoneyBigDecimal(u.total(), symbol),
                    ...
                    item {
                        LuisoCard(modifier = Modifier.fillMaxWidth()) {
                            DetailRow("MONEDA", "$symbol (${u.currencyCode})")
                        }
                    }

                    item {
                        LuisoCard(modifier = Modifier.fillMaxWidth()) {
                            DetailRow("VENTAS", "${u.count}")
                        }
                    }
                    ...
                    if (u.products.isNotEmpty()) {
                        ...
                        items(u.products, key = { "${it.name}-${it.unit}" }) { item ->
                            UnitedProductLine(item = item, symbol = symbol)
                        }
                    }
                    ...
                    if (u.items.isEmpty()) {
                        ...
                        items(u.items, key = { "${it.denominationValue}" }) { item ->
                            UnitedItemLine(item = item, symbol = symbol)
                        }
                    }
                        ...
                        Column(modifier = Modifier.fillMaxWidth()) {
                            ...
                            DropdownMenu(
                                ...
                                DropdownMenuItem(text = { Text("PDF") }, onClick = {
                                    ...
                                    PdfExporter(context).exportUnited(u, symbol)
                                })
                                DropdownMenuItem(text = { Text("Excel (CSV)") }, onClick = {
                                    exportMenuOpen = false
                                    ExcelExporter(context).exportUnited(u, symbol)
                                })
                            }
                        }
                    }
